...
T7:    bne $s1, 7, T8
        ...
        move $a1, $v0 #put the value into $a1        
        
        move $a0, $s0#put the pointer 
...
T8:     bne $s1, 8, T9
        ...
        move $a0, $v0 ## put the (possibly revised) pointer into s0
        
        #next step is to display the duplicated list
        
        jal displayDuplicatedList
...
T10:	bne $s1,10, T8no	# if s1 = 7, do these things. Else go to T7no
...
Insert_n:
        li $v0,-1
	beq $a0,0,insert_n_return
	
	move $t0,$a0	# linked list head
	move $t1,$a1	# new number to add end of list
	#subi $t2,$t2, 1
	move $t2,$a2	# n
	move $t4,$a0	# head
	...
	sw $t1,4($v0)	# node->number = number NEW NODE'S DATA IS ASSIGNED
	insert_loop:
	lw $t3,0($t0)				# load head->next
	beq $t3,0,insert_loop_end	# if head->next = null
	subi $t2,$t2,1  # n=n-1  LIKE A COUNTER IT DECREASES THE INDEX WHEN TRACING
	beq $t2,1,insert_n_operation
	lw $t0,0($t0)			# head = head->next;  LIKE A TRACER
	j insert_loop

insert_n_operation:
	lw $t3,0($t0)				# load head ->next 
	sw $t3,0($v0)				# node->next = head->next
	
insert_loop_end:
	sw $v0,0($t0)		# head->next = newNode
	li $v0,0
insert_n_return:
	move $v1, $t4
	jr $ra
...
deleteAfter_x:
	lw $t0,4($a0)			# get head value
	beq $t0,$a1,deleteHead
	move $t0,$a0 			# t0 = currentPointer
deleteXLoop:
	lw $t1,0($t0)			# next pointer
	beq $t1,0,invalidNumber		# if next == NULL
	lw $t2,4($t1)			# next pointer value
	beq $a1,$t2,deleteXLoopEnd	
	lw $t0,0($t0)
	j deleteXLoop
deleteXLoopEnd:
        lw $t0, 0($t0) #ben
...
deleteHead:
	li $v0, 0
	move $v1, $a0
	lw $v1,0($v1)			#head = head->next
	jr $ra


duplicateListIterative: 
        
        move $t4, $a0 #copy the head pointer
        beq  $t4, $0, returnNull
        bne $t4, $0, copyHead
...
msg1171: .asciiz "7-  insert element into nth position of the linked list \n"

msg118:  .asciiz "10 - exit this program \n"
msg1181: .asciiz "8- duplicate list iteratively \n"

msgRecursive: .asciiz"9- duplicate list recursively \n"
...
msgInsert: .asciiz "Enter the position that you want to insert element: "
msgInsert1: .asciiz"Enter the element that you want to insert into that position: "

duplicate: .asciiz "List is being duplicated \n"
